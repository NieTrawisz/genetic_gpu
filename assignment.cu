...
struct larger_tuple
{
        __device__ __host__
            thrust::tuple<T, int>
            operator()(const thrust::tuple<T, int> &a, const thrust::tuple<T, int> &b)
        {
                if (a > b)
                        return a;
                else
...
int main(int argc, char **argv)
{
        ...
        while (best < .999)
        {
                ...
                init<<<SIZE_PARENT_POOL, POPULATION_SIZE>>>(time(0), states);
                gpuErrchk(cudaPeekAtLastError());

                setRandom<<<SIZE_PARENT_POOL, POPULATION_SIZE>>>(states, randParents, POPULATION_SIZE);
                gpuErrchk(cudaPeekAtLastError());
